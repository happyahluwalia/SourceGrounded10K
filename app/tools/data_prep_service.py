"""
data_prep_tool - Orchestrates the Complete SEC Filing Processing Pipeline
...
--------------------
The 10-k Financial Annual Report tool coordinates 5 main services:
...
-------------
```python
from app.tools.data_prep_tool import data_prep_tool
...
data_prep_tool = data_prep_tool(db_storage, vector_store)

# Process a filing (fetches if missing)
result = data_prep_tool.get_or_process_filing("AAPL", "10-K")
...
class DataPrepTool:
    ...
    Example:
        >>> # Initialize service
        >>> service = 10kFinancialAnnualReportPipelineTool(db_storage, vector_store)
        >>> 
        >>> # Process a filing
    ...
    def __init__(
        ...
        Example:
            ...
            >>> db = DatabaseStorage()
            >>> vs = VectorStore()
            >>> service = 10kFinancialAnnualReportPipelineTool(db, vs)
        """
